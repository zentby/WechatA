...
function WeChatApi(setting){
	...
	var _access_token = setting.access_token || { Token: '', ExpireAt: Date.now()};
	...
	this.createCall = function(method, path, options, callback) 
	{
        return function() 
        {
            ...
            logger.info('api is called: '+JSON.stringify(parameters));
            request(parameters, function(err, res, body) {
                logger.info('api is returned: '+body);
	...
	{
	    var urlapi = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid=' + _appId
	    + '&secret=' + _appSecret + '&code=' + code + '&grant_type=authorization_code';
